...
// ===== Real ADC read =====
// Uses analogReadMilliVolts() to read actual sensor voltage
...
void startOffsetCollection() {
    long sA = 0, sB = 0, sC = 0;
    for (int i = 0; i < 50; i++) {
        sA += analogReadMilliVolts(PIN_INA);
        sB += analogReadMilliVolts(PIN_INB);
        sC += analogReadMilliVolts(PIN_INC);
        delay(2);
    }
    offsetA = sA / 50.0f;
    offsetB = sB / 50.0f;
...
void udpTask(void *pvParameters) {
    while (1) {
        if (isMeasuring && pendingPackets > 0) {
            ...
            float rA = analogReadMilliVolts(PIN_INA), rB = analogReadMilliVolts(PIN_INB), rC = analogReadMilliVolts(PIN_INC);
